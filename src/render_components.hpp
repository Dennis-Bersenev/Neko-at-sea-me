...
//this for debugging while the other is to show the bounding/collision stuff 
struct DebugModeComponent
{
	// Note, an empty struct has size 1
};

//this for pausing
struct PauseModeComponent
{
	// Note, an empty struct has size 1
};

//this for pausing
struct OptionModeComponent
{
	// Note, an empty struct has size 1
};

//this for end
struct EndModeComponent
{
	// Note, an empty struct has size 1
};

// A timer for moving to a different scene
struct TransitionTimer
{
	float counter_ms = 1000;
};

// A timer for showing the achievements
struct AchievementTimer
{
