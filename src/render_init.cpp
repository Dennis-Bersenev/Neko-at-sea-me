	...
	window(window)
{
	...
	// For texturing game screen
	// Making the texture
	if (debug)
	{
		std::string s = std::to_string(1);
		std::string texture_path = "data/background_bumps/" + s + ".png";
		int width, height, channels;
		stbi_uc* data = stbi_load(texture_path.c_str(), &width, &height, &channels, 0);

		if (data == NULL)
			throw std::runtime_error("data == NULL, failed to load texture");

		glGenTextures(1, &game_textures[0]);
		glBindTexture(GL_TEXTURE_2D, game_textures[0]);
		gl_has_errors();
	...
	else
	{
		for (int i = 0; i < 120; i++)
		{
			...
			glGenTextures(1, &game_textures[i]);
			glBindTexture(GL_TEXTURE_2D, game_textures[i]);
			gl_has_errors();
	...
	std::cout << "Rendering initialized, textures loaded up!\n";
	return;
}
...
// Create a new sprite and register it with ECS
void RenderSystem::createSpriteSheet(ShadedMesh& sprite, std::string texture_path, std::string shader_name, float frames, float rows)
{
	if (texture_path.length() > 0)
		sprite.texture.load_from_file(texture_path.c_str());

	float animation_frame_width = (sprite.texture.size.x / frames) / sprite.texture.size.x;
	float animation_frame_height = (sprite.texture.size.y / rows) / sprite.texture.size.y;
	...
	vertices[2].position = { +1.f / 2, -1.f / 2, 0.f };
	vertices[3].position = { -1.f / 2, -1.f / 2, 0.f };
	vertices[0].texcoord = { 0.f, animation_frame_height };
	vertices[1].texcoord = { animation_frame_width, animation_frame_height };
	vertices[2].texcoord = { animation_frame_width, 0.f };
	vertices[3].texcoord = { 0.f, 0.f };
...
void RenderSystem::initScreenTexture()
{
	...
	createSprite(screen_sprite, "", "screen_tex");
